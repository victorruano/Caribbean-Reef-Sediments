...
pf.ra.loc<-read.csv("Large_parrots.csv")
pf.4<-read.csv("Parrotfish_size_4.csv")
...
dplot.pf.spp<-ggplot(data=pfsize, 
                       aes(x=Size_class, 
                           group=Locality, 
                           fill=Locality))+
  geom_density(adjust=1.5, 
...
# stacked bar plot per locality for large parrotfish
stack.large.p<-ggplot(pf.ra.loc, aes(x = Locality, y = Rel_abundance, fill = Species)) +
  scale_fill_manual(values = c("firebrick", "coral", "goldenrod", "olivedrab", "steelblue", "purple3"))+
  geom_col() +
...
PFish_Size_RA<-ggarrange(dplot.pf.local, stack.large.p,
          labels = c("A", "B"),
          ncol = 1, nrow = 2)

ggsave("PFish_Size_RA.tiff", plot = PFish_Size_RA, 
       width = 4.5, height = 7.2, units = "in", dpi = 300)
...
# Rugosity
rugosity<-read.csv("Rugosity.csv")
shapiro.test(rugosity$Rugosity)
bartlett.test(rugosity$Rugosity~rugosity$Locality)
bartlett.test(rugosity$Rugosity~rugosity$Site)

summary(aov(rugosity$Rugosity~rugosity$Locality+Error(rugosity$Site)))
summary(aov(rugosity$Rugosity~rugosity$Locality+rugosity$Site))

################## Sediments multivariate analysis ######################
library(vegan)
...
sed.lme.cc<-lme(Coral_cover~Coral_grain, 
                data=sed.eros, random = ~1|Locality/Site)
summary(sed.lme.cc)
...
# Halimeda
hal.lme<-lme(Halimeda_cover~Halimeda_grain, data=sed.eros, random = list(~Locality|Site))
summary(hal.lme)

# Logistic regression
sed.eros.glme2<-glmer(Bioerosion~Coral_grain+(1|Locality:Site),
                      family=binomial,data=sed.eros.binomal)
summary(sed.eros.glme2)
